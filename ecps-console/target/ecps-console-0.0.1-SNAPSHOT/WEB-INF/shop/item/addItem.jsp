...
<div id="tab_3" class="edit set" style="display: none">
    <c:if test="${fn:length(itemFeatures) == 0}">
    <p><label></label>无属性</p>
    </c:if>
	<c:forEach items="${itemFeatures}" var="feature">
		<p><label>${feature.featureName }:</label>
			<c:if test="${feature.inputType == 1 }">
				<select name="">
					<option value="">请选择</option>
					<!-- forEach可以把以逗号分隔的字符串作为一个集合来遍历， 集合中的每一项就是字符串以逗号分隔的每一项 -->
			...
			<c:if test="${feature.inputType == 2 }">
				<input type="radio" name="${feature.featureName }">无&nbsp;
				<c:forEach items="${feature.selectValues }" var="val">
					<input type="radio" name="${feature.featureName }" value="${val }">${val }
				</c:forEach>
			</c:if>
			<c:if test="${feature.inputType == 3 }">
				<c:forEach items="${feature.selectValues }" var="val">
					<input type="checkbox" name="${feature.featureName }" value="${val }">${val }
				</c:forEach>
			</c:if>
			<c:if test="${feature.inputType == 4 }">
					<input type="text" name="${feature.featureName }" value="${feature.selectValues }">
			</c:if>
		</p>
	</c:forEach>
</div>
...
<div id="tab_4" style="display:none">
    ...
    <div id="sp_0" class="sp_0">
        <table cellspacing="0" summary="" class="tab3">
            <c:if test="${fn:length(skuFeatures) == 0}">
            <tr><th colspan="2" class="gray b">&nbsp;&nbsp;<b>默认</b></th></tr>
            </c:if>
            <c:forEach items="${skuFeatures }" var="feature">
	           <tr>
