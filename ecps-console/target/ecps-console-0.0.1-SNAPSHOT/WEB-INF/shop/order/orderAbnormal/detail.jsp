...
<head>
<title>订单详情_异常单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
$(function(){
	...
	$(".sub1").click(function(){
		...
		$("#errorInfoAdd5").html("<label>&nbsp;</label>");
	    $("#itemNote5").val("");
	    tipHide("#errorInfoAdd5");
	    var d=$("#addItemNote5 h2").attr("title","系统提示").html("系统提示");
		tipShow('#addItemNote5');
	});
	$(".sub2").click(function(){
		tipShow('#jobNums');
	});
	$(".sub3").click(function(){
		...
		a.append('<input type="hidden" name="note" id="x"/>');
		$("#x").val(note);
		a.append('<input type="hidden" name="jobNum"  value="'+jobNum+'" id="_jobNum" />');
		a.append('<input type="hidden" name="r"  value="'+obj.val()+'" id="_r" />');
		$.ajax({
			type : "POST",
			async : false,
			url : "${path}/order/checkJobNum.do",
			data:{
				jobNum:jobNum,
				username:username,
				orderNum:orderNum
	...
	$("#r").click(function(){
	    ...
	    $("#inputIMSI").css("width", "170px");
	    $("#inputICCID").css("width", "170px");
	    if(${ebco.ebOrder.orderType == 3}){
	    	tipShow("#pIMSI");
	    } 
	    if(${ebco.ebOrder.orderType == 3}){
	    	tipShow("#pIMSI");
	    	$("#sIMSI").html("串号：");
	    	$("#inputIMSI").val(${IMSICode });
	    }
	    if(${ebco.ebOrder.orderType == 2}){
		    tipShow("#pICCID");
		  //  $("#inputIMSI").val("${IMSICode }");
		    $("#inputICCID").val("${ICCIDCode }");
	    }
	...
	$("#but1").click(function(){
	    ...
	    if(oprType=="2"){
	    	window.location.href="${path}/order/orderAbnormal.do?orderState=9"
	...
	$("input[id='addItemNoteConfirm']").click(function(){
		...
		if(${ebco.ebOrder.orderType==3}){
			var _txt = "${ebco.ebOrder.orderType==3?'串号':'IMSI'}";
			var sn=$("#inputIMSI").val();
			if(!(/^[0-9]{15}$/.test(sn))) {
				alert(_txt + "应输入15位数字！ ");
				return;
			}
	...
	$("input[id='addItemNoteConfirm5']").click(function(){
		var note=$("#itemNote5").val();
		var a=$("#myForm");
		if(note.length>200){
			tipShow("#errorInfoAdd5");
			$("#errorInfoAdd5").html("<label>&nbsp;</label>操作备注不能大于200个字符");
			return;
		}
		a.append('<input type="hidden" name="note" id="x"/>');
		$("#x").val(note);
		...
		a.submit();
        tipHide('#addItemNote5');
	});
	$("#addItemNoteClose5").click(function(){
		tipHide('#addItemNote5');
		$("#itemNote5").val("");
	});
	$("#addItemNoteReset5").click(function(){
		tipHide('#addItemNote5');
		$("#itemNote5").val("");
	});
	...
	$("#getIccidLink").click(function(){
		$.ajax({
			type : "POST",
			async : false,
			url : "${path}/order/preemptSimCardRes.do",
			data : {serviceNum:$("#serviceNum").val(),areaCode:$("#areaCode").val(),orderSource:$("#orderSource").val()},
			success : function(responseText) {
				if(responseText!=null){
					$("#inputICCID").val(responseText);
				}
			},
			error : function() {
				alert("查询ICCID时出错");
...
function crm(note) {
	...
	var r=$("#r").val();
	var imsiCode = $("#inputIMSI").val();
	var iccidCode = $("#inputICCID").val();
	$.ajax({
		type : "POST",
		async : false,
		url : "${path}/order/doAjaxCrm1.do",
		data:{
			orderNum:orderNum,
			oprType:oprType,
			note:note,
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;异常单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>
<form id="myForm"action="${path}/order/orderAbnormal/oper.do" method="post">
   ...
   <div class="sch page_c">
    	<span class="l">
    	订单号：<b class="red"<var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="1"/></b>&nbsp;&nbsp;&nbsp;
    	 ...
    	 <c:if test="${ebco.ebOrder.orderState == 12 && oprType !=1}">
	    	 <ui:permTag src="/ecps/console/order/orderAbnormal/oper.do">
	           <input name="r" type="button" class="pointer sub1"  value="退库处理"/>
            </ui:permTag>
            </c:if>
             <c:if test="${ebco.ebOrder.orderState == 9 && oprType !=1}">

			<ui:permTag src="/ecps/console/order/doAjaxCrm1.do">
				<%-- <input type="button" name="r" class="pointer" onclick="getQuery(${ebco.ebOrder.orderNum});"value="查询办理"/> --%>
            	<input name="r" type="button" class="pointer" id="r" value="重新办理"/>
			</ui:permTag>
			<ui:permTag src="/ecps/console/order/orderAbnormal/oper.do">
			<input name="r" type="button" class="pointer sub2"  value="手动开通"/>
            <input name="r" type="button" class="pointer sub1"  value="推至外呼"/>
				</ui:permTag>
...
<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=1','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
</h2>
 ...
 <div id="tab_1">
	<!-- 基本信息 -->
		<table cellspacing="0" summary="" class="tab4">
			...
			<tr>
				<th width="12%">下单用户：</th>
				<td id="username" width="21%" class="nwp">${ebco.ebOrder.userName}</td>
				<th width="12%">支付方式:</th>
				<c:choose>
					<c:when test="${ebco.ebOrder.orderSource == 1}">
			...
			<table cellspacing="0" summary="" class="tab3">
				...
				<c:forEach items="${ebco.ebOrderSimDtail}" var="e">
					<tr>
						...
						<td class="nwp">
							号码: <c:out value="${e.itemName}"></c:out><input type="hidden" id="serviceNum" value="${e.itemName}"></br>
							归属地: <c:out value="${ebco.ebOrder.areaName}"></c:out><input type="hidden" id="areaCode" value="${ebco.ebOrder.areaCode}"></br>
