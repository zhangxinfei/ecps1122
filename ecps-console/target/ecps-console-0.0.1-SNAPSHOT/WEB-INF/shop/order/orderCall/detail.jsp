...
<head>
<title>订单详情_外呼单_<fmt:message key="OrderMgmtMenu.title" /></title>
...
$(function(){
	...
	$("#confirmDivOk").bind("click",function(){
		var a=$("#myForm");
		var a1=$("#phone").val();
		var a11=$("#fixedPhone").val();
		if (a1 == "" && a11 == "") {
			alert("联系电话或固定电话必须填一项 ");
			return ;
		}
		if(!(/^[0-9]{11}$/.test(a1))&&a1.length!=0){
			alert("联系电话：请输入11位数字 ");
			return ;
		}
		if(!(/^[0-9-]{0,20}$/.test(a11))&&a11.length!=0){
			alert("只允许输入0-20位固定电话，如\"010-3882316\"");
			return ;
		}
		var a2=$("#zip").val();
		if(!(/^\d{6}$/.test(a2))&&a2.length!=0){
			alert("邮政编码：请输入6位数字 ");
			return ;
		}
		var a3=$("#addr").val();
		if(a3.length>200){
			alert("收货地址：请输入200位以内字符 ");
			return ;
		}
		var a4=$("#note").val();
		if(a4.length>200||a4.length==0){
			alert("外呼备注：请输入200位以内字符 ");
			return ;
			}
		var a5=$("#shipName").val();
		if(a5.length>20||a5.length==0){
			alert("收货人：请输入20位字符 ");
			return ;
		}
//			if($(this).val() == "推迟外呼"){
...
//				} 
	...
	$("input[id='addItemNoteConfirm']").click(function(){
		...
		if(itemNote.length>200){
			...
			$("#errorInfoAdd").html("<label>&nbsp;</label>外呼备注不能大于200个字符");
			return;
		}
		var a1=$("#phone").val();
		var a11=$("#fixedPhone").val();
		...
		var a3=$("#addr").val();
		if(a3.length>200||a3.length==0){
			alert("收货地址：请输入200位以内字符 ");
			return ;
		}
// 		var a4=$("#note").val();
// 		if(a4.length>200){
// 			alert("外呼备注：请输入200位字符 ");
// 			return ;
// 			}
		var a5=$("#shipName").val();
		if(a5.length>20||a5.length==0){
...
<body id="main">
	...
	<div class="frameR"><div class="content">
				<div class="loc icon">
					...
					<c:if test="${oprType==null}">
						<samp class="t12"></samp>
						<fmt:message key='menu.current.loc' />：<fmt:message
							key='OrderMgmtMenu.title' />&nbsp;&raquo;&nbsp;外呼单&nbsp;&nbsp;<span
							class="gray" title="订单详情"></span>
					</c:if>
				</div>
				
				<form id="myForm" action="${path}/order/orderCall/oper.do"
					method="post">
					...
					<div class="sch page_c">
						<span class="l">订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="2"/></b>&nbsp;&nbsp;&nbsp; 
						...
						<c:if test="${(ebco.ebOrder.orderState == 1 ||ebco.ebOrder.orderState == 30 ) && oprType !=1}">
						<ui:permTag src="/ecps/console/order/orderCall/oper.do">
							<input name="r" type="button" class="pointer sub1 blue" value="推迟外呼" />
							<c:if test="${ebco.ebOrder.orderSource != 1}">
								<input name="r" type="button" class="pointer sub1 red" value="作废并申请退款" /> 
							</c:if>
							<input name="r" type="button" class="pointer sub1 blue" value="通过" />
						</ui:permTag>
						</c:if>						
							<c:if test="${(ebco.ebOrder.orderState == 21||ebco.ebOrder.orderState == 31 ) && oprType !=1}">
						<input name="r" type="button" class="pointer sub1 blue" value="推迟外呼" />
						<input name="r" type="button" class="pointer sub1 red" value="作废并申请退款" /> 
						</c:if>						 
	...
	<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=2','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表" /></span>
	</h2>
...
<!-- 基本信息 -->
		<table cellspacing="0" summary="" class="tab4">
			...
			<c:choose>
				<c:when test="${!((ebco.ebOrder.orderState == 1 ||ebco.ebOrder.orderState ==21||ebco.ebOrder.orderState ==31 ||ebco.ebOrder.orderState ==30)&& oprType !=1)}">
					<tr>
						<th>收货人：</th>
				...
				<c:otherwise>
					<tr>
						<th><samp class="red">*</samp>收货人：</th>
						<td class="nwp"><input id="shipName" name="shipName" type="text" value="<c:out value="${ebco.ebOrder.shipName}"/>" class="text small2"></input></td>
						<th><samp class="red">*</samp>联系电话：</th>
					...
					<tr>
						<th><label><samp class="red">*</samp></label>收货地址：</th>
						<td class="nwp" colspan="3">
						<c:out value="${ebco.ebOrder.province}"/>&nbsp;<c:out value="${ebco.ebOrder.city}"/>&nbsp;<c:out value="${ebco.ebOrder.district}"/>&nbsp;
						<input id="addr" name="addr" type="text" value="<c:out value="${ebco.ebOrder.addr}"/>" class="text large" />
						</td>
						<th>邮编：</th>
						<td><c:if test="${ebco.ebOrder.deliveryMethod == 1}">
							<input id="zip" name="zip" type="text" value="${ebco.ebOrder.zipCode}" class="text small2" /></c:if>
							<c:if test="${ebco.ebOrder.deliveryMethod == 2}">
							<input id="zip" name="zip" type="text" class="text small2" /></c:if>
						</td>
					</tr>
				</c:otherwise>
			</c:choose>
