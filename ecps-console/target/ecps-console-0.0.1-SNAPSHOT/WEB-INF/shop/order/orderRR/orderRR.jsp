...
<head>
<title>申请退换货_<fmt:message key="OrderMgmtMenu.title"/></title>
...
function exportOrder(s){
    ...
    form.submit();
    form.action="${path}/order/orderRR.do";
}
</script>
...
<div class="frameR"><div class="content">
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key="OrderMgmtMenu.title"/>&nbsp;&raquo;&nbsp;<span class="gray" title="申请退换货">申请退换货</span></div>
<form action="${path}/order/orderRR.do" id="form1" name="form1" method="post">
   ...
   <table cellspacing="0" summary="" class="tab" id="myTable">
    ...
    <tr>
    <td><a href="${path}/order/orderRR/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">${p.orderNum}</a></td>
    <td>${p.shipName}</td>
    <td>${p.phone}</td>
    ...
    <td class="nwp">已收货</td>
    <td><a href="${path}/order/orderRR/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">查看</a></td>
    </tr>
    </c:forEach>
    </table>
    <div class="page_c">
