...
<head>
<title>订单详情_退库单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
$(function(){
	...
	$(".sub1").click(function(){
		obj=$(this);
		$("#returnNoteAdd").html("<label>&nbsp;</label>");
		$("#returnNote").val("");
	    $("#attachFilePath").val("");
	    tipHide("#returnNoteAdd");
		tipShow('#returnNoteDiv');
	});
	var r;
	$("#r").click(function(){
		r=$("#r").val();
	    tipHide("#returnNoteAdd");
	    $("#attachFileAttention").show();
	    $("#attachFileField").show();
	    <c:if test="${ebco.ebOrder.orderType == 2 || ebco.ebOrder.orderType == 3}">
	    	$("#pJobNum").show();
	    	$("#iJobNum").val("${jobNum}");
	    </c:if>
	    <c:if test="${!(ebco.ebOrder.orderState == 37 || ebco.ebOrder.orderState == 43)}">
	    	$("#attachFileAttention").hide();
	    	$("#attachFileField").hide();
	    </c:if>
		tipShow('#returnNoteDiv');
	});
	$("#r1").click(function(){
	...
	$("#returnNoteClose").click(function(){
		tipHide('#returnNoteDiv');
		$("#returnNote").val("");
	});
	$("#returnNoteReset").click(function(){
		tipHide('#returnNoteDiv');
		$("#returnNote").val("");
	});
	...
	$("#attachFileDelete").click(function(){
		deleteUploadsAttach();
	});
	$("#but1").click(function(){
		var oprType=$("#oprType").val();
	    if(oprType=="2"){
	    	window.location.href="${path}/order/orderValid.do?orderState=13"
	    	
	    } else {
	    	window.location.href="${path}/order/orderRstock.do?orderState=13"
	    }
	});
	$("input[id='returnNoteConfirm']").click(function(){
		
		var a=$("#myForm");
		<c:if test="${ebco.ebOrder.orderType == 2 || ebco.ebOrder.orderType == 3}">
			var reg = new RegExp('^[0-9a-zA-Z]*$');
			var jobNum = $("#iJobNum").val();
			if($.trim(jobNum)==''){
				alert('工单号不能为空');
				return false;
			}
		...
		var itemNote=$("#returnNote").val();
		if(itemNote.length>200){
			tipShow("#returnNoteAdd");
			$("#returnNoteAdd").html("<label>&nbsp;</label>操作备注不能大于200个字符");
			return;
		}
        tipHide('#returnNoteDiv');
        tipShow("#refundLoadDiv");
        setTimeout(function(){crm(r);}, 500);
	});
});
...
function crm(r) {
	var note = $("#returnNote").val();
	var oprType=$("#oprType").val();
	var orderNum=$("#orderNum").val();
	var taskId=$("#taskId").val();
	var attachFile = $('#attachFilePath').val();
	var jobNum = $("#iJobNum").val();
	$.ajax({
		type : "POST",
		async : false,
		url : "${path}/order/cancelAjaxCrm.do",
		data:{
			orderNum:orderNum,
			oprType:oprType,
			note:note,
		...
		success : function(responseText) {
			...
			if(oprType=="2"){
		    	...
		    	window.location.href="${path}/order/orderRstock.do?orderState=13,36,37,43";
		    }
		}
	});
}

function checkAttachFileType(type) {
	var allowFileTypes = new Array(/^\.(z|Z)(i|I)(p|P)$/,/^\.(r|R)(a|A)(r|R)$/);
...
function submitAttachUploads() {
	...
	if (!checkAttachFileType(type)) {
		alert("只允许上传格式为zip、rar的图片");
		return;
	}
	
	var uploadUrl = $('#attachUploadAction').val();
	var options = {
			beforeSubmit: showAttachUploadsRequest,
			success:      showAttachUploadsResponse,
			type:         'post',
			dataType:     "script",
			data:{
				'fileObjName':'attach'
	...
	$('#attachFileForm').ajaxSubmit(options);
}

function showAttachUploadsRequest(formData, jqForm, options) {
	return true;
}

function showAttachUploadsResponse(responseText, statusText, xhr, $form) {
	...
	var filePath = responseText[0].relativePath;
	var fileName = responseText[0].fileName;
	$("#attachFileDisplayName").text(fileName);
	$("#attachFileDisplay").show();
	$('#attachFilePath').attr("value", filePath);
}

function deleteUploadsAttach() {
	/* var filePath = $('#attachFilePath').val();
	var deleteUrl = $('#attachDeleteAction').val();
	$.ajax({
		type:"post",
		url:deleteUrl,
		dataType:"json",
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;退库单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>
<form id="myForm" action="${path}/order/orderRstock/oper.do" method="post">
   <input type="hidden" name="taskId" id="taskId" value="${taskId}"/>
    ...
    <input type="hidden" name="attachFile" id="attachFilePath" value=""/>
    
    <input type='hidden' id='attachUploadAction' name='attachUploadAction' value='${path}/uploads/upload_attach.do' />
    <input type='hidden' id='attachDeleteAction' name='attachDeleteAction' value='${path}/uploads/upload_delete.do' />

 <div class="sch page_c">
    	<span class="l">
    	订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="9"/></b>&nbsp;&nbsp;&nbsp;
    	 ...
    	 <c:if test="${(ebco.ebOrder.orderState == 13 || ebco.ebOrder.orderState == 36 || ebco.ebOrder.orderState == 37 || ebco.ebOrder.orderState == 43) && oprType !=1}">
    	 <ui:permTag src="/ecps/console/order/orderRstock/oper.do">
    	 	<c:if test="${ebco.ebOrder.orderType == 2 || ebco.ebOrder.orderType == 3}">
    	 		<input type="button" name="r" class="pointer" id="r" value="已退库并发起退款，解除营销案"/>
    	 		<input type="button" name="r1" class="pointer" id="r1" value="已退库并发起退款，手工解除营销案"/>
    	 	</c:if>
            <c:if test="${ebco.ebOrder.orderType == 1 || ebco.ebOrder.orderType == 4 || ebco.ebOrder.orderType == 5}">
            	<input type="button" name="r" class="pointer" id="r" value="已退库并发起退款"/>
            </c:if>
			</ui:permTag>
    	</c:if>
    	</span>
	...
	<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=9','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
  ...
  <div id="tab_1">
		...
		<table cellspacing="0" summary="" class="tab4">
			...
			<c:if test="${ebco.ebOrder.orderState == 40 || ebco.ebOrder.orderState == 41 || ebco.ebOrder.orderState == 22 || ebco.ebOrder.orderState == 44 || ebco.ebOrder.orderState == 23 || ebco.ebOrder.orderState == 46}">
				<tr>
					<th>质检报告</th>
					<td>
						<c:choose>
							<c:when test="${not empty ebco.ebOrder.attachFile}">
								<a href="${rsImgUrlInternal}${ebco.ebOrder.attachFile}" title="质检报告" class="nor">下载</a>
							</c:when>
					...
					<td>
						<c:choose>
							<c:when test="${ebco.ebOrder.returnType==0}">
								后台
							</c:when>
							<c:when test="${ebco.ebOrder.returnType==1}">
								前台
							</c:when>
	...
	<c:choose>
		...
		<c:when test="${ebco.ebOrder.orderType == 1 || ebco.ebOrder.orderType == 4 || ebco.ebOrder.orderType == 5}">
			<table cellspacing="0" summary="" class="tab3">
				...
				<c:forEach items="${ebco.ebOrderItemDetail}" var="e" varStatus="status">
				<tr>
					<td>${e.itemNo}</td>
					...
					<td>${e.quantity}个</td>
					<td>${e.seriescode}</td>
					<%-- <c:if test="${status.index==0 && (ebco.ebOrder.orderState == 40 || ebco.ebOrder.orderState == 41 || ebco.ebOrder.orderState == 22 || ebco.ebOrder.orderState == 44 || ebco.ebOrder.orderState == 23 || ebco.ebOrder.orderState == 46)}">
						<td rowspan="${fn:length(ebco.ebOrderOfferDetail)}">
							<c:choose>
								<c:when test="${not empty ebco.ebOrder.attachFile}">
									<a href="${rsImgUrlInternal}${ebco.ebOrder.attachFile}" title="质检报告" class="nor">下载</a>
								</c:when>
...
<div id="tab_2" style="display: none">
	<table cellspacing="0" summary="" class="tab3">
		...
		<tr><th width="12%">付款类型：</th><td>${epr.poName}</td></tr>
