...
<head>
<title>取货单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
// 	 case "6":$("#label4").attr("class","here");break;
// 	 case "7,8,22,23,25":$("#label5").attr("class","here");break;
// 	 case "12,13,14,19,20,24":$("#label6").attr("class","here");break;
	}
...
function exportOrder(s){
    ...
    form.action="${path}/order/orderExport.do?s="+s+"&type=2&b=C7";
    subVerify4Title($("#orderNo"));
    subVerify4Title($("#phone"));
    subVerify4Title($("#shipName"));
    subVerify4Title($("#userName"));
    form.submit();
    form.action="${path}/order/orderDelivery.do";
}
...
<div class="frameR"><div class="content">
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key="OrderMgmtMenu.title"/>&nbsp;&raquo;&nbsp;<span class="gray" title="取货单">取货单</span></div>
    <form action="${path}/order/orderDelivery.do" id="form1" name="form1" method="post">
    <h2 class="h2_ch">
      <span id="tabs" class="l">
        <a id="label1" href="${path}/order/orderDelivery.do"   title="全部" class="nor">全部</a>
        <a id="label2" href="${path}/order/orderDelivery.do?orderState=4"   title="待打包" class="nor">待打包</a>  
        <a id="label3" href="${path}/order/orderDelivery.do?orderState=5"   title="待物流取货" class="nor">待物流取货</a>  
<%--         <a id="label4" href="${path}/order/orderDelivery.do?orderState=6"   title="配送中" class="nor">配送中</a>   --%>
<%--         <a id="label5" href="${path}/order/orderDelivery.do?orderState=7,8,22,23,25"   title="配送成功" class="nor">配送成功</a> --%>
<%--         <a id="label6" href="${path}/order/orderDelivery.do?orderState=12,13,14,19,20,24" style="color:red;" title="配送失败" class="nor">配送失败</a> --%>
      </span>
      <span class="r inb_a">
   ...
   <table cellspacing="0" summary="" class="tab" id="myTable">
    ...
    <tr>
    <td>${p.areaName}</td>
    <td><a href="${path}/order/orderDelivery/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">${p.orderNum}</a></td>
    <td>${p.shipName}</td>
    <td>${p.phone}</td>
    ...
    <td><a href="${path}/order/orderDelivery/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">查看</a></td>
    </tr>
    </c:forEach>
    </table>
    <div class="page_c">
