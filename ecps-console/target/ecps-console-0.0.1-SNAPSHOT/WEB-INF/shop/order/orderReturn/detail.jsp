...
<head>
<title>订单详情_退款单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;退款单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>
<form id="myForm" action="${path}/order/orderReturn/oper.do" method="post">
	...
	<div class="sch page_c">
	   	<span class="l">订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}" pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="8" /></b>&nbsp;&nbsp;&nbsp;
		...
		<ui:permTag src="/ecps/console/order/orderReturn/oper.do">
		<c:if test="${ebco.ebOrder.orderState == 7 && oprType !=1}">
		<input name="r" class="pointer sub1" type="button" value="发起退款"/>
		</c:if>
		<c:if test="${(ebco.ebOrder.orderState == 38||ebco.ebOrder.orderState == 39||ebco.ebOrder.orderState == 40||ebco.ebOrder.orderState == 41||ebco.ebOrder.orderState == 15||ebco.ebOrder.orderState == 26) && oprType !=1}">
		<input name="r" class="pointer sub1" type="button" value="同意退款"/>
		</c:if>
		<c:if test="${(ebco.ebOrder.orderState == 40||ebco.ebOrder.orderState == 41) && oprType !=1}">
		<input name="r" class="pointer sub1" type="button" value="拒绝"/>
		</c:if>
<%-- 		<c:if test="${(ebco.ebOrder.orderState == 10||ebco.ebOrder.orderState == 20||ebco.ebOrder.orderState == 22||ebco.ebOrder.orderState == 33) && oprType !=1}"> --%>
<!-- 		<input name="r" class="pointer sub1" type="button" value="已退款"/> -->
<%-- 		</c:if> --%>
		</ui:permTag>
		</span>
	</div>
	...
	<a href="javascript:void(0);" ref="#tab_3" title="操作记录" class="nor">操作记录</a>
	 ...
	 <span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=8','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
	...
	<!-- 金额合计 -->
		<div class="page_c">
			<c:if test="${ebco.ebOrder.orderState == 24||ebco.ebOrder.orderState == 25||ebco.ebOrder.orderState == 26||ebco.ebOrder.orderState == 15||ebco.ebOrder.orderState == 10
			||ebco.ebOrder.orderState == 20||ebco.ebOrder.orderState == 22||ebco.ebOrder.orderState ==33}">
			...
			<c:if test="${ebco.ebOrder.orderState == 11||ebco.ebOrder.orderState == 14||ebco.ebOrder.orderState == 23||ebco.ebOrder.orderState == 34}">
			<span class="r">商品金额合计：<fmt:formatNumber value="${ebco.ebOrder.orderSum/100}" pattern="#0.00"/>元&nbsp;&nbsp;&nbsp;
					 运费：0.00元 &nbsp;&nbsp;&nbsp; 已退金额： <var class="f16 red"><fmt:formatNumber value="${ebco.ebOrder.orderSum/100}" pattern="#0.00"/>元</var>
			</span></c:if>
		</div>
