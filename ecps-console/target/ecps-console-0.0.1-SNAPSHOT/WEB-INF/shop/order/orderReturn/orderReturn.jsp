...
<head>
<title>退款单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
// 	 case "7":$("#label2").attr("class","here");break;
	 case "15,26,38,39,40,41":$("#label3").attr("class","here");break;
	 case "13,36,37,43":$("#label6").attr("class","here");break;
	 case "10,20,22,33,44,45":$("#label5").attr("class","here");break;
	 case "11,14,23,34,46,47":$("#label4").attr("class","here");break;
	}
});
function exportOrder(s){
    var form = document.getElementById("form1");
    form.action="${path}/order/orderExport.do?s="+s+"&type=2&type2=8";
    subVerify4Title($("#orderNo"));
    subVerify4Title($("#phone"));
    subVerify4Title($("#shipName"));
    subVerify4Title($("#userName"));
    form.submit();
    form.action="${path}/order/orderReturn.do";
}
</script>
...
<div class="frameL"><div class="menu icon">
    <jsp:include page="/ecps/console/common/ordermenu.jsp"/>
</div></div>

<div class="frameR"><div class="content">
    <div class="loc icon"><samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key="OrderMgmtMenu.title"/>&nbsp;&raquo;&nbsp;<span class="gray" title="退款单">退款单</span></div>
    <h2 class="h2_ch">
      <span id="tabs" class="l">
        <a id="label1" href="${path}/order/orderReturn.do"   title="全部" class="nor">全部</a>
<%--         <a id="label2" href="${path}/order/orderReturn.do?orderState=7"   title="已收货可退款" class="nor">已收货可退款</a>   --%>
        <a id="label3" href="${path}/order/orderReturn.do?orderState=15,26,38,39,40,41"   title="退款待审核" class="nor">退款待审核</a>
        <a id="label6" href="${path}/order/orderReturn.do?orderState=13,36,37,43"   title="退款审核未通过" class="nor">退款审核未通过</a>
        <a id="label5" href="${path}/order/orderReturn.do?orderState=10,20,22,33,44,45"   title="待退款" class="nor">待退款</a>
        <a id="label4" href="${path}/order/orderReturn.do?orderState=11,14,23,34,46,47" style="color:red;" title="已退款" class="nor">已退款</a>
      </span>
       <span class="r inb_a">
    ...
    <form action="${path}/order/orderReturn.do" id="form1" name="form1" method="post">
    ...
    <tr>
    <td>${p.areaName}</td>
    <td><a href="${path}/order/orderReturn/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">${p.orderNum}</a></td>
    <td>${p.shipName}</td>
    <td>
    	<c:choose>
    ...
    <td class="nwp"><ui:orderModule var="${p.orderState}" type="8"/></td>
    <td><a href="${path}/order/orderReturn/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">查看</a></td>
    </tr>
    </c:forEach>
    </table>
    <div class="page_c">
