...
<head>
<title>订单详情_作废单_<fmt:message key="OrderMgmtMenu.title"/></title>
	...
	*/
	$("#confirmDivOk").bind("click",function(){
		...
		$.ajax({
			type : "POST",
			async : false,
			url : "${path}/order/convertOutTimeOrderToPaidOrder.do",
			data:{
				taskId:taskId,
				orderNum:orderNum
			},
			success : function(responseText) {
				window.location.href = "${path}/order/orderInValid.do?orderState=17";
			}
		});
	});
});
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;作废单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>

<form id="myForm" action="${path}/order/orderInValid/oper.do" method="post">
    <input type="hidden" id="taskId" name="taskId" value="${taskId}"/>
    <input type="hidden" id="userId" name="userId" value="${userId}"/>
    <input type="hidden" id="orderNum" name="orderNum" value="${ebco.ebOrder.orderNum}"/>

	<div class="sch page_c">
	   	<span class="l">
	   	 订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="6"/></b>&nbsp;&nbsp;&nbsp;
...
<!-- 		<input name="r" type="button" class="hand btn83x23" value="恢复" onclick="confirm(this.value);"/> -->
		</c:if>
		<ui:permTag src="/ecps/console/order/convertOutTimeOrderToPaidOrder.do">
			<input name="r" type="button" class="hand btn83x23"  value="恢复为已支付单" onclick="confirm(this.value);"/>
		</ui:permTag>
	...
	<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=6','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
