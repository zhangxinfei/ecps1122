...
<head>
<title>订单详情_配送单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
$(".sub2").click(function(){
	    ...
	    $("#itemNote").val("");
	    var d=$("#deliverycall h2").attr("title","配送备注").html("配送备注");
		tipShow('#deliverycall');
		var a=$("#myForm");
		return;
});
$("#deliverycallClose").click(function(){
	tipHide('#deliverycall');
	$("#itemNote1").val("");
});
...
<form id="myForm" action="${path}/order/orderDelivery1/oper.do" method="post">
...
<%--             </c:if> --%>
	<ui:permTag src="/ecps/console/order/orderDelivery1/oper.do">
   <c:if test="${ebco.ebOrder.orderState == 4 && oprType !=1}">      
            ...
            <c:if test="${(ebco.ebOrder.orderState == 6||ebco.ebOrder.orderState == 32) && oprType !=1}">      
            <!--             <input name="r" type="button" class="hand btn83x23 sub1" value="配送成功"/>  -->
<!--             <input name="r" type="button" class="hand btn83x23 sub1" value="配送失败"/>  -->
			<input type="button" class="pointer sub2 " value="外呼确认"/></c:if>
			</ui:permTag>
    	</span>
	</div>
...
<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=5','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
</h2>
  <div id="tab_1">
<!-- 基本信息 -->
