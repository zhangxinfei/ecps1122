...
<head>
<c:choose>
	<c:when test="${ebco.ebOrder.orderSource == 1 && flag == 1}">
		<title>代客下单管理</title>
		<meta name="heading" content="<fmt:message key='mainMenu.heading'/>"/>
		<meta name="menu" content="relpaceGuestSubmitOrder"/>
	</c:when>
	<c:otherwise>
		<title>订单详情_订单总览_<fmt:message key="OrderMgmtMenu.title"/></title>
		<meta name="heading" content="<fmt:message key='mainMenu.heading'/>"/>
...
<div class="loc icon">
	<c:choose>
		<c:when test="${ebco.ebOrder.orderSource == 1 && flag == 1}">
			<samp class="t12"></samp><fmt:message key='menu.current.loc'/>：代客下单管理&nbsp;&raquo;&nbsp;查询/修改订单&nbsp;&raquo;&nbsp;<span class="gray" title="查看订单详情">查看订单详情</span>
		</c:when>
		<c:otherwise>
...
<form id="myForm" action="${path}/order/orderStock/oper.do" method="post">
    ...
    <div class="sch page_c">
    	<span class="l">
    	订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderState var="${ebco.ebOrder.orderState}" type="1"/></b>
	...
	<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=13','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
