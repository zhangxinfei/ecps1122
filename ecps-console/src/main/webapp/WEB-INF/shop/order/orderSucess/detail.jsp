...
$(function(){
	...
	$("#but1").click(function(){
		...
		if(isSuccess == 'false'){
		    ...
		    if(oprType=="2"){
		    	window.location.href="${path}/order/orderValid.do?orderState=7,42"
		    	
		    } else {
		    	window.location.href="${path}/order/orderSucess.do?orderState=7,42"
		    }
		}
	});
...
function crm() {
	...
	$.ajax({
		type : "POST",
		async : false,
		url : "${path}/order/cancelAjaxCrm1.do",
		data:{
			orderNum:orderNum,
			oprType:oprType,
			note:note,
...
<form id="myForm" action="${path}/order/orderSucess/oper.do" method="post">
   <input type="hidden" name="taskId" id="taskId" value="${taskId}"/>
	...
	<div class="sch page_c">
		 ...
		 <ui:permTag src="/ecps/console/order/orderSucess/oper.do">
	...
	<a href="javascript:void(0);" ref="#tab_3" title="操作记录" class="nor">操作记录</a>
	 ...
	 <span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=7','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes')" value="打印订单" /><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
