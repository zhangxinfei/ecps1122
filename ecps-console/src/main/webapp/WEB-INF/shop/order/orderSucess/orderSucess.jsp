...
function exportOrder(s){
    ...
    form.action="${path}/order/orderExport.do?s="+s+"&type=2&type2=7";
    subVerify4Title($("#orderNo"));
    subVerify4Title($("#phone"));
    subVerify4Title($("#shipName"));
    subVerify4Title($("#userName"));
    form.submit();
    form.action="${path}/order/orderSucess.do";
}
</script>
...
<%--         <a id="label1" href="${path}/order/orderSucess.do"   title="全部" class="nor">全部</a> --%>
        <a id="label2" href="${path}/order/orderSucess.do?orderState=7,42"   title="已收货" class="nor">已收货</a>  
<%--         <a id="label3" href="${path}/order/orderReturn.do?orderState=24,25,26"   title="待审核退款" class="nor">待审核退款</a> --%>
<%--         <a id="label5" href="${path}/order/orderReturn.do?orderState=10,20,22"   title="待退款" class="nor">待退款</a> --%>
<%--         <a id="label4" href="${path}/order/orderReturn.do?orderState=11,14,23" style="color:red;" title="已退款" class="nor">已退款</a> --%>
      </span>
       <span class="r inb_a">
    ...
    <form action="${path}/order/orderSucess.do" id="form1" name="form1" method="post">
    ...
    <tr>
    <td>${p.areaName}</td>
    <td><a href="${path}/order/orderSucess/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">${p.orderNum}</a></td>
    <td>${p.shipName}</td>
    <td>
    	<c:choose>
    ...
    <td><a href="${path}/order/orderSucess/detail.do?orderId=${p.orderId}&taskId=${p.taskId}">查看</a></td>
    </tr>
    </c:forEach>
    </table>
    <div class="page_c">
