...
<head>
<title>订单详情_申请退换货_<fmt:message key="OrderMgmtMenu.title"/></title>
...
$(function(){
	...
	$(".sub1").click(function(){
	    ...
	    if(obj.val()=="申请退货"){
	    	str="退货原因";
	    }else if(obj.val()=="申请换货"){
	    	str="换货原因";
	    }
	    var d=$("#addItemNote h2").attr("title",str).html(str);
		tipShow('#addItemNote');
	});
	$("input[id='addItemNoteConfirm']").click(function(){
		...
		a.append('<input type="hidden" name="desc"  value="'+itemNote+'"/>');
...
<div class="loc icon">
    <c:if test="${oprType == '1'}">
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;订单总览&nbsp;&nbsp;<span class="gray" title="订单详情">订单详情</span>
    </c:if>
    <c:if test="${oprType == '2'}">
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;有效订单&nbsp;&nbsp;<span class="gray" title="订单详情">订单详情</span>
    </c:if>
    <c:if test="${oprType==null}">
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;申请退换货&nbsp;&nbsp;<span class="gray" title="订单详情">订单详情</span>
    </c:if>
</div>
<form id="myForm" action="${path}/order/orderRR/oper.do" method="post">
    <input type="hidden" name="taskId" value="${taskId}"/>
    <input type="hidden" name="userId" value="${userId}"/>
    <input type="hidden" name="oprType" value="${oprType}"/>
    <input type="hidden" name="shipName" value="${ebco.ebOrder.shipName}"/>
    <input type="hidden" name="phone" value="${ebco.ebOrder.phone}"/>
    <input type="hidden" name="orderNum" value="${ebco.ebOrder.orderNum}"/>

	<div class="sch page_c">
	   	<span class="l">
	   	 订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue">已收货</b>&nbsp;&nbsp;&nbsp;
	   	 <c:if test="${ebco.ebOrder.orderState == 15 && oprType !=1}">
	           <input type="hidden" name="r1" value="配送中"/>
	           <ui:permTag src="/ecps/console/order/orderRR/oper.do">
	           <input name="r" type="button" class="hand btn83x23 sub1" value="申请退货"/>
	           <input name="r" type="button" class="hand btn83x23 sub1" value="申请换货"/>
	           </ui:permTag>
	           </c:if>
	   	</span>
	</div>
...
<%-- <div class="page_c">
<c:if test="${ebco.ebOrder.orderState == 15 && oprType !=1}">
        <h2 class="h2" title="退换货描述">退换货描述</h2>
        <div class="edit set"><samp>*</samp><textarea id="desc" name="desc" class="are arew"></textarea></div>
	</c:if>
		</div> --%>
