...
<head>
<title>订单详情_取货单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
$(".sub2").click(function(){
	    ...
	    $("#itemNote").val("");
	    var d=$("#deliverycall h2").attr("title","配送失败备注").html("配送失败备注");
		tipShow('#deliverycall');
		var a=$("#myForm");
		return;
});
...
$(".sub3").click(function(){
	var a=$("#myForm");
	obj=$(this);
	var itemNote1=$("#itemNote1").val();
	if(itemNote1.length>200){
		tipShow("#errorInfoAdd1");
		$("#errorInfoAdd1").html("<label>&nbsp;</label>备注不能大于200个字符");
		return;
	}else{
	a.append('<input type="hidden" name="note"  value="'+itemNote1+'"/>');
	...
	//ddd
// 	$("input[id='faultdis']").click(function(){
// // 		if(obj==null){
// // 			return;
// // 		}
// 		var a=$("#myForm");
// 		var itemNote1=$("#itemNote1").val();
// 		if(itemNote1.length>5){
// 			tipShow("#errorInfoAdd");
// 			$("#errorInfoAdd").html("<label>&nbsp;</label>备注不能大于200个字符");
// 			return;
// 		}
// 		a.append('<input type="hidden" name="note"  value="'+itemNote1+'"/>');
//        // tipHide('#addItemNote');
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;配送单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>
<form id="myForm" action="${path}/order/orderDelivery/oper.do" method="post">
  ...
  <div class="sch page_c">
    	<span class="l">
    	 订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="5"/></b>&nbsp;&nbsp;&nbsp;
    	 ...
    	 <c:if test="${ebco.ebOrder.deliveryNo != null}">物流编号:${ebco.ebOrder.deliveryNo}</c:if>
<%--     	<c:if test="${ebco.ebOrder.orderState == 4 && oprType !=1}"> --%>
<!--             <input type="hidden" name="r1" value="待取货"/> -->
<!--             <input name="r" type="button" class="hand btn83x23 sub1" value="配送成功"/> -->
<!--             <input name="r" type="button" class="hand btn83x23 sub1" value="配送失败"/> -->
<%--             </c:if> --%>
	<ui:permTag src="/ecps/console/order/orderDelivery/oper.do">
   <c:if test="${ebco.ebOrder.orderState == 4 && oprType !=1}">      
            输入运输编号：<input id="deliveryNo" name="deliveryNo" type="text"
					value="${ebco.ebOrder.deliveryNo}" class="text small2" />
            <input name="r" type="button" class="hand btn83x23 sub1" value="打包完成"/>
          </c:if>
            <c:if test="${ebco.ebOrder.orderState == 5 && oprType !=1}">      
            输入运输编号：<input id="deliveryNo" name="deliveryNo" type="text"
					value="${ebco.ebOrder.deliveryNo}" class="text small2" />
            <input name="r" type="button" class="hand btn83x23 sub1" value="物流已取货"/>
            
            </c:if>
            <c:if test="${ebco.ebOrder.orderState == 6 && oprType !=1}">      
            <!--             <input name="r" type="button" class="hand btn83x23 sub1" value="配送成功"/>  -->
<!--             <input name="r" type="button" class="hand btn83x23 sub1" value="配送失败"/>  -->
			<input type="button" class="hand btn83x23 sub2 " value="外呼确认"/></c:if>
			</ui:permTag>
    	</span>
	</div>
...
<span class="r"><input type="button" class="hand btn80x20" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}','_blank','height=300,width=500,scrollbars=yes,location=no')" value="打印订单" /><input type="button" class="hand btn80x20" onclick="window.history.back(-1);" value="返回列表"/></span>
</h2>
  <div id="tab_1">
<!-- 基本信息 -->
		<table cellspacing="0" summary="" class="tab4">
			<tr>
				...
				<th width="12%">
					<c:if test="${ebco.ebOrder.orderType == 2 && ebco.ebOrder.orderSource == 2 }">
						订单来源
					</c:if>
				</th>
				<td class="nwp">
					<c:if test="${ebco.ebOrder.orderType == 2 && ebco.ebOrder.orderSource == 2 }">
						<c:out value="套卡"></c:out>
					</c:if>
				</td>
			</tr>
			<tr>
				...
				<th width="12%">用户备注:</th>
				<td class="nwp" colspan="3">${ebco.ebOrder.notes}</td>
			</tr>
...
<div id="tab_2" style="display: none">
	<table cellspacing="0" summary="" class="tab3">
		<tr><th width="12%">支付流水号：</th><td>${ebco.ebOrder.paymentNo}</td></tr>
