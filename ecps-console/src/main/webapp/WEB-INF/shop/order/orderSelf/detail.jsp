...
<head>
<title>订单详情_自提单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;自提单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>

<form id="myForm" action="${path}/order/orderSelf/oper.do" method="post">
   ...
   <div class="sch page_c">
    	<span class="l">
    	 订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="11"/></b>&nbsp;&nbsp;&nbsp;
    	 <c:if test="${ebco.ebOrder.orderState == 35 && oprType !=1}">
            <input type="hidden" name="r1" value="配送中"/>
            <ui:permTag src="/ecps/console/order/orderSelf/oper.do">
            <input name="r" type="button" class="pointer" onclick="confirm(this.value);" value="自提完成"/>
            <input name="r" type="button" class="pointer" onclick="confirm(this.value);"  value="自提退库"/>
            <!-- <input name="r" type="button" class="hand btn83x23b" onclick="confirm(this.value);"  value="回退"/> -->
            </ui:permTag>
            </c:if>
    	 </span>
	</div>
	...
	<span class="r"><input type="button" class="pointer" onclick="window.open('${base}/order/preview.do?orderId=${ebco.orderId}&type=11','_blank','height=300,width=500,scrollbars=yes,location=no,resizable=yes');" value="打印订单"/><input type="button" class="pointer" onclick="window.history.back(-1);" value="返回列表"/></span>
	</h2>
