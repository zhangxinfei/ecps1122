...
<head>
<title>订单详情_写卡单_<fmt:message key="OrderMgmtMenu.title"/></title>
...
<div class="loc icon">
    ...
    <samp class="t12"></samp><fmt:message key='menu.current.loc'/>：<fmt:message key='OrderMgmtMenu.title'/>&nbsp;&raquo;&nbsp;写卡单&nbsp;&nbsp;<span class="gray" title="订单详情"></span>
    </c:if>
</div>
<form id="myForm" action="${path}/order/orderCard/oper.do" method="post">
    ...
    <div class="sch page_c">
    	<span class="l">
    	 订单号：<b class="red"><var>${ebco.ebOrder.orderNum}</var></b>&nbsp;&nbsp;&nbsp;下单时间：<var><fmt:formatDate value="${ebco.ebOrder.orderTime}"pattern="yyyy-MM-dd HH:mm:ss" /></var>&nbsp;&nbsp;&nbsp;<b class="f14 blue"><ui:orderModule var="${ebco.ebOrder.orderState}" type="3"/></b>&nbsp;&nbsp;&nbsp;
    	<c:if test="${ebco.ebOrder.orderState == 4 && oprType !=1}">
            <input name="r" type="button" class="hand btn83x23 sub1"  value="写卡"/>
            </c:if>
            <c:if test="${ebco.ebOrder.orderState == 5 && oprType !=1}">
            <input name="r" type="button" class="hand btn83x23 sub1"  value="写卡成功"/>
            <input name="r" type="button" class="hand btn83x23 sub1"  value="写卡异常"/>
            </c:if>
    	</span>
	</div>
